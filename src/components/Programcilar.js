...
export default function Programcilar() {
  ...
  const programmer = programcilar.find(item=>activeId==item.id)
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activeId? "gold":'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setActiveId(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
